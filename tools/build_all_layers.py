...
# ------------------------------------------------------------------
# 2. Body Layers (Layer 3) - Horizontally flipped & standardized collar
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 3. Face Shapes (Layer 4) - Pure Blank Smooth Face Bases (Plan B)
# ------------------------------------------------------------------
print('3. Face Shapes (10 natural + 10 tan clean smooth bases)...')
for i in range(1, 11):
    for tone in ['natural', 'tan']:
        ...
        # Smooth nose (remove highlight and shadow lines in nose box)
        ...
        # Smooth mouth (remove lips and smile lines without touching chin/jaw)
...
# ------------------------------------------------------------------
# 4. Eyes Layer (Layer 7) - Crisp transparent extraction
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 8. Bangs Layer (Layer 9) - Full Cranial Coverage & Clean Foreheads
# ------------------------------------------------------------------
print('8. Bangs Layer (16 styles x 2 colors)...')
bangs_cat = Image.open(os.path.join(PHOTOS_DIR, 'bangs_16_catalog.jpg'))
bang_cards = [
    # Male 1..6
    ('bang_01', (130, 68, 280, 230), 220),
    ('bang_02', (420, 68, 570, 230), 220),
    ('bang_03', (710, 68, 860, 230), 220),
    ('bang_04', (145, 288, 285, 450), 220),
    ('bang_05', (420, 288, 580, 450), 220),
    ('bang_06', (715, 288, 855, 450), 220),
    # Female 1..10
    ('bang_07', (35, 555, 220, 725), 245),
    ('bang_08', (220, 555, 405, 725), 245),
    ('bang_09', (430, 565, 590, 725), 225),
    ('bang_10', (625, 565, 785, 725), 225),
    ('bang_11', (815, 565, 975, 725), 235),
    ('bang_12', (45, 795, 205, 950), 220),
    ('bang_13', (230, 795, 395, 950), 235),
    ('bang_14', (420, 795, 585, 950), 235),
    ('bang_15', (615, 795, 785, 950), 235),
    ('bang_16', (810, 795, 970, 950), 220),
]
...
for name, box, target_w in bang_cards:
    ...
    for y in range(card.height):
        for x in range(card.width):
            r, g, b = card.getpixel((x, y))
            if max(r, g, b) < 160:
                if abs(x - card.width * 0.5) < 35 and y > card.height * 0.60:
                    continue
                b_rgba.putpixel((x, y), (r, g, b, 255))
    bbox = b_rgba.getbbox()
    if bbox:
...
# ------------------------------------------------------------------
# 9. Back Hair Layer (Layer 2) - Behind Head (Periphery Only)
# ------------------------------------------------------------------
...
for name, box in back_cards:
    ...
    for y in range(card.height):
        for x in range(card.width):
            p = card.getpixel((x, y))
            if max(p) < 135:
                is_face = (x >= 45 and x <= 135 and y >= 55 and y <= 160)
                is_ear = (x >= 120 and x <= 155 and y >= 95 and y <= 165)
                is_jaw_neck = (x >= 25 and x <= 140 and y >= 140)
                if 'bob' in name or 'wavy' in name:
                    is_jaw_neck = (x >= 48 and x <= 130 and y >= 140)
                    is_ear = False
                if not (is_face or is_ear or is_jaw_neck):
...
print('All 10 layers rebuilt with Plan B high precision!')
